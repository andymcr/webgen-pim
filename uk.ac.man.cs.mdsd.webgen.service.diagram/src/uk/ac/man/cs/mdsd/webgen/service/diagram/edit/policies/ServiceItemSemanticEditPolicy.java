 ...
 * @generated
 */
public class ServiceItemSemanticEditPolicy extends ServiceBaseItemSemanticEditPolicy {
	...
	*/
	private void addDestroyChildNodesCommand(ICompositeCommand cmd) {
		...
		for (Iterator<?> nit = view.getChildren().iterator(); nit.hasNext();) {
			...
			switch (ServiceVisualIDRegistry.getVisualID(node)) {
			case ServiceSelectionCompartmentEditPart.VISUAL_ID:
				for (Iterator<?> cit = node.getChildren().iterator(); cit.hasNext();) {
					Node cnode = (Node) cit.next();
					switch (ServiceVisualIDRegistry.getVisualID(cnode)) {
